...
public class CustomerServiceImpl implements CustomerService {
	...
	@Autowired
	private BCryptPasswordEncoder pwdEncoder;  // it is used to encrypt passwords before saving and Matches the passwords during login.
	...
	public Boolean register(CustomerDto customDto) {
		...
		String ogPwd = generteRandomPassword();
		...
		if(savedEntity.getId() != null) {
			...
			String subject = "Registration successfull - Welcome to our Platform!";
           ...
           try {
			 emailService.sendEmail(customDto.getCustomerEmail(), subject, body);
           }catch(Exception e) {
			   e.printStackTrace();
			   return false;
		   }
	...
	public Boolean isEmailUnique(String emailId) {
		Customer byEmail = customerRepo.findByCustomerEmail(emailId);
		return byEmail == null;
		
		// this method is used to finds customer by Email and return true if the email is not used.
	}

	@Override
	public Boolean resetPassword(ResetPwdDto pwdDto) {
		
		Customer c = customerRepo.findByCustomerEmail(pwdDto.getCustomerEmail());
		
		if( c != null) {
//			String newPassword = pwdDto.getNewPassword();
			String encodedPwd = pwdEncoder.encode(pwdDto.getNewPassword());
	...
	public CustomerDto getCustomerByEmail(String emailId) {
		...
		Customer c = customerRepo.findByCustomerEmail(emailId);
	...
	public AuthResponse login(CustomerDto customDto) {
	    ...
	    UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(
	                    customDto.getCustomerEmail(),
	                    customDto.getPassword()
	            );
	    ...
	    if (authenticate.isAuthenticated()) {
	        ...
	        Customer c = customerRepo.findByCustomerEmail(customDto.getCustomerEmail());
	        authResponse = new AuthResponse();
	        authResponse.setCustomer (CustomerMapper.convertToDto(c));
	        authResponse.setToken(""); // JWT later
	    }

	    return authResponse;
	}

     private String generteRandomPassword() {
